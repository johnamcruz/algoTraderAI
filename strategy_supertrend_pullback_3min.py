#!/usr/bin/env python3
"""
Supertrend Pullback Strategy Implementation (V3.9 - Transformer)
...
class SupertrendPullbackStrategy(BaseStrategy):
    """
    Supertrend Pullback V3.9 (Transformer) trading strategy.
    Uses "Pure AI Mode" - all filters are learned by the model.
    """

    def __init__(self, model_path: str, scaler_path: str, contract_symbol: str):
        """
        Initialize SupertrendPullbackStrategy (V3.9)
        """
        super().__init__(model_path, scaler_path, contract_symbol)
        logging.info("Initialized SupertrendPullbackStrategy (V3.9 - Pure AI)")


    def get_feature_columns(self) -> List[str]:
        """
        Returns the 25 feature columns for the V3.9 Supertrend Transformer.
        (Matches SUPERTRND PULLBACK AI - V3.9)
        """
        return [ 
             ...
             # --- NEW V3.9 TREND CLARITY FEATURES ---
             'st_slope_long',
             'dist_to_ema200',
             'adx_acceleration_5'
        ]
    ...
    def get_sequence_length(self) -> int:
        """
        Supertrend Pullback V3.9 (Transformer) uses 120 bars.
        """
        return 120

    # =========================================================
    # V3.9 FEATURE ENGINEERING
    # (Matches add_supertrend_features_v3_9 from training)
    # =========================================================
    
    def add_features(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Calculate Supertrend Pullback V3.9 features.
        (Matches add_supertrend_features_v3_9 from training)
        """        
        logging.debug(f"Adding V3.9 Supertrend features. Input shape: {df.shape}")
        ...
        # --- DATA POISONING FIX (Matches Training) ---
        # Replace inf, -inf with NaN
        df.replace([np.inf, -np.inf], np.nan, inplace=True)
        
        # Use ffill, bfill, and fillna(0) to prevent dropna() from
        # removing rows, which is critical for live bot execution.
        ...
        logging.debug(f"V3.9 Supertrend features added. Shape after features: {df.shape}")
        return df
    ...
    def load_model(self):
        ...
        try:
            ...
            logging.info(f"✅ Loaded Supertrend Pullback V3.9 model: {os.path.basename(self.model_path)}")
        except Exception as e:
            logging.exception(f"❌ Error loading Supertrend Pullback V3.9 model: {e}")
            raise
    ...
    def load_scaler(self):
        ...
        try:
            ...
            if base_symbol in scalers:
                ...
                logging.info(f"✅ Loaded '{base_symbol}' scaler for Supertrend V3.9")
            else:
            ...
            logging.exception(f"❌ Error loading Supertrend V3.9 scaler: {e}")
            raise

    # --- (No change to predict() function, it's correct) ---
    def predict(self, df: pd.DataFrame) -> Tuple[int, float]:
        """
        Generate prediction using V3.9 Transformer model.
        ...
        try:
            ...
            logging.exception(f"❌ Prediction error (Supertrend V3.9): {e}")
    ...
    def should_enter_trade(
        ...
        """
        Determine if V3.9 "Pure AI Mode" entry conditions are met.
        
        REMOVED: ADX, EMA, and Proximity filters.
        KEPT: Supertrend direction check, as this is fundamental to the
              "pullback" label definition the model was trained on.
        """
