#!/usr/bin/env python3
"""
CISD+OTE Strategy v10.0 — FFM Hybrid Transformer

Changes from v7.0:
  - Backbone: v5 → v6 (structure label horizon fix 20 → 48 bars)
  - FFM features: 67 → 68 (adds htf_daily_structure at index 63)
  - Context heads: 4 frozen pretrained heads (regime/vol/structure/range) concat
    15-dim softmax to fusion input — baked into ONNX, no new inputs required
  - Bull P/D filter: sweep override removed for bull CISD (only bears bypass P/D
    gate when had_sweep=1, matching v10 training — USE_SWEEP_OVERRIDE_PD bears only)
  - Session fallback boundaries updated: pre<07:00, london 07–09:30, ny_am 09:30–13:00
...
# ── CISD detection constants (must match cisd_ote_v10.py training) ───────────
SWING_PERIOD        = 6
TOLERANCE           = 0.70
EXPIRY_BARS         = 50
...
class CISDOTEStrategyV10(BaseStrategy):
    """
    CISD+OTE Hybrid Strategy using FFM Transformer backbone (v10.0).

    Architecture:
      FFMBackbone(256-dim, 68 features × 96 bars)
      + 4 frozen context heads (regime/vol/structure/range → 15-dim softmax)
      + CISDProjection(10 features)
      → signal_logits [B, 2]       apply softmax for signal prob
        risk_predictions [B, 1]    predicted max R:R (Softplus)
    ...
    P/D filter (matches v10 training):
      Bull CISD: allowed only when in discount zone
      Bear CISD: allowed when in premium zone OR had_liquidity_sweep=1
    """
    ...
    def __init__(
        ...
        logging.info("=" * 65)
        logging.info("🎯 CISD+OTE Strategy v10.0 — FFM Hybrid Transformer")
        logging.info("=" * 65)
        logging.info(f"  seq_len=96 | FFM features=68 | CISD features=10")
        logging.info(f"  Backbone: v6 | Context heads: 4 frozen (15-dim softmax)")
    ...
    def add_features(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Compute 68 FFM features via derive_features() and update CISD zone state.
    ...
    def load_model(self):
        ...
        if missing:
            raise ValueError(
                ...
                f"CISDOTEStrategyV10. Missing required inputs: {sorted(missing)}. "
                f"Model has: {sorted(input_names)}. "
                f"Use models/cisd_ote_hybrid_v10.onnx with this strategy."
            )
    ...
    def predict(self, df: pd.DataFrame) -> Tuple[int, float]:
        ...
        try:
            ...
            # ── session_ids [1, 96] — pre_market=0 london=1 ny_am=2 ny_pm=3 ─
            # Boundaries (ET): pre<07:00 | london 07:00–09:30 | ny_am 09:30–13:00 | ny_pm 13:00+
            if 'sess_id' in df.columns:
                sess = df['sess_id'].values.astype(np.int64)
            elif hasattr(df.index, 'hour'):
                mins = df.index.hour * 60 + df.index.minute
                sess = np.where(mins < 7 * 60, 0,
                       np.where(mins < 9 * 60 + 30, 1,
                       np.where(mins < 13 * 60, 2, 3))).astype(np.int64)
            else:
            ...
            logging.debug(
                f"  CISD+OTE v10 | conf={confidence:.3f} "
                f"dir={'BUY' if prediction==1 else 'SELL' if prediction==2 else 'NONE'}"
            )
            return prediction, confidence

        except Exception as e:
            logging.exception(f"❌ CISD+OTE v10 predict error: {e}")
            return 0, 0.0
    ...
    def should_enter_trade(
        ...
        if prediction == 1:
            logging.info(f"✅ CISD+OTE v10 BUY  | conf={confidence:.3f} rr={self._latest_risk_rr:.2f}")
            return True, 'LONG'
        elif prediction == 2:
            logging.info(f"✅ CISD+OTE v10 SELL | conf={confidence:.3f} rr={self._latest_risk_rr:.2f}")
            return True, 'SHORT'
    ...
    def _update_cisd_detector(self, df: pd.DataFrame, abs_bar: int):
        """
        Incremental CISD zone detection.

        P/D filter matches v10 training (USE_PD_FILTER=True, USE_SWEEP_OVERRIDE_PD=True):
          Bull CISD: allowed only when in discount (no sweep override)
          Bear CISD: allowed when in premium OR had_sweep=1
        """
        n   = len(df)
        bar = n - 1
        ...
        # Track sweep (wick-throughs of active pivots)
        new_ph = []
    ...
    def _build_cisd_feature_vector(self, df: pd.DataFrame, abs_bar: int) -> Optional[np.ndarray]:
        """
        Build the 10-element CISD feature vector matching CISD_FEATURE_COLS from training.
        """
        if not self._active_zones:
            self._latest_zone_bullish = 0.0
            return None
