#!/usr/bin/env python3
"""
CISD+OTE Strategy Implementation (v5.5) — FFM Hybrid Transformer
...
Key differences from v5.1:
  - cisd_features vector is 32 elements (was 28); indices 28-31 are new
    regime/vol context features required by the v5.5 model weights
  - Entry thresholds: >=0.85 standard (was >=0.70), >=0.90 for 4R sizing
  - Two ONNX inputs: 64-bar FFM sequence + 32-element point-in-time CISD vector
...
Walk-forward results (v5.5 @ >=0.85 signal_prob):
  Combined: 37 trades/backtest | PF 6.06 | AvgRR 4.62
  @ >=0.90: 21 trades/backtest | PF 6.56 | AvgRR 4.92
"""
...
class CISDOTEStrategyV2(BaseStrategy):
    """
    CISD+OTE Hybrid Strategy using FFM Transformer backbone (v5.5).

    Architecture:
      FFMBackbone (256-dim, 42 features) + CISDProjection (32 features)
    ...
    def __init__(
        ...
        # ── v5.5 regime tracking (new features idx 28-31) ──
        # htf_trend_strength (idx 29): consecutive bars HTF trend has held
        # regime_transition_recency (idx 31): decays 1.0 -> 0 over 48 bars after flip
        self._htf_prev_direction: float = 0.0
        self._htf_trend_run_bars: int = 0
        self._bars_since_htf_flip: int = 999  # large = old/no flip

        logging.info("=" * 65)
        logging.info("🎯 CISD+OTE Strategy v5.5 — FFM Hybrid Transformer")
        logging.info("=" * 65)
        logging.info(f"  SEQ_LEN: 64 bars | CISD features: 32")
    ...
    def add_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'str_structure_state' in df.columns:
            ...
            if current_htf != self._htf_prev_direction:
                ...
                self._htf_prev_direction = current_htf
            else:
                self._htf_trend_run_bars += 1
                self._bars_since_htf_flip += 1

        # ── Run incremental CISD detector on latest bar ──
        self._update_cisd_detector(df, self._bar_count)

        # ── Build 32-element CISD feature vector for latest bar ──
    ...
    def should_enter_trade(
        ...
        Recommended thresholds (walk-forward v5.5):
          0.85 → standard entry,  PF 6.06, 37 trades/backtest
          0.90 → 4R target sizing, PF 6.56, 21 trades/backtest
          0.70 → forward-test comparison vs v5.1 only (not for live)
    ...
    def _compute_ffm_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Store atr14 and atr_ma50 as helpers for CISD feature 28
        df['_atr14']    = atr14
        df['_atr_ma50'] = atr_ma50
        ...
        # abs bar delta rolling SMAs — used for CISD feature 30 (delta_momentum_ratio)
        abs_delta = np.abs(bar_delta)
    ...
    def _build_cisd_feature_vector(self, df: pd.DataFrame, abs_bar: int) -> Optional[np.ndarray]:
        """
        Build the 32-element CISD feature vector for the latest bar.
        ...
        # 28: atr_compression_ratio = ATR14 / 50-bar ATR mean (same as vty_regime)
        atr_compression = ffm('vty_regime', default=1.0)

        # 29: htf_trend_strength = consecutive HTF trend bars / 96
        htf_trend_strength = min(self._htf_trend_run_bars / 96.0, 1.0)

        # 30: delta_momentum_ratio = 5-bar abs-delta SMA / 20-bar abs-delta SMA
        d5  = ffm('_abs_delta_5sma',  default=1.0)
        d20 = ffm('_abs_delta_20sma', default=1.0)
        delta_momentum = d5 / max(d20, 1e-8)

        # 31: regime_transition_recency = 1.0 at HTF flip, decays to 0 over 48 bars
        regime_recency = max(0.0, 1.0 - (self._bars_since_htf_flip / 48.0))
        ...
        features = np.array([
            ...
            np.clip(delta_momentum, 0, 10),               # 30: delta_momentum_ratio
            np.clip(regime_recency, 0, 1),                # 31: regime_transition_recency
        ], dtype=np.float32)
